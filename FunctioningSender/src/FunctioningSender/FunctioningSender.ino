...
byte data[32] = {"12345678901939739587293857"};                           //Data buffer
...
unsigned int TX=1,RX=0,role=0,lastrole=0,SKIP=2,RXPRINT=3,RXDUMP=4;
...
void loop(void){
  if(role == TX){
    delay(250);                                              // Pause for a couple seconds between transfers
    printf("Initiating Extended Timeout Data Transfer\n\r");
    unsigned long cycles = 10;                              // Change this to a higher or lower number. This is the number of payloads that will be sent.
